...
\lhead{Capítulo 1. \emph{Introduction}}
...
Unfortunately, some of the semantics for the C language described in the standard\cite{c99} are prone to ambiguities due to the use of the English language to describe program behavior.
...
According to Tennent\cite{tennent}, in order to support and define these meanings for a construct in a programming language a rigorous mathematical theory of the semantics of programming languages is needed.

As noted by Nielson and Nielson\cite{nielson}, the rigorous nature of this study is due to the fact that it can reveal ambiguities or underlying complexities in documents defined using natural language, and also that this mathematical rigor is necessary for correctness proofs.
...
In order to further clarify the definition and the different types of semantics we are going to use an example we consider relevant from Nielson and Nielson\cite{nielson}.
...
It can be thought of as an abstraction of the execution of the program in a machine\cite{nielson}.
...
This kind of semantics represents the execution of a program from an initial state to a final state in one single step, therefore it does not allow for explicit inspection of intermediate execution states\cite{nipkow}.
...
However we can obtain the following ``derivation sequence'' for the previous construct:
...
This kind of semantics represents small, atomic execution steps in a program and allows for reasoning about how far a program has been executed and to explicitly inspect partial executions\cite{nipkow}.
...
This approach assists us by giving a \textit{meaning} to the constructs in a language\cite{nipkow}.
...
Examples of this are variable initialization, constant folding and reachability\cite{nielson}.
...
It is usually easier to talk about the concept of partial correctness\cite{nipkow}.
...
This approach relies on a \textit{proof system} or inference rules for deriving and proving partial correctness properties\cite{nipkow}.
...
By using a theorem prover, in our case Isabelle/HOL\cite{isabelle-tutorial}, one can be certain that the results proved are correct.
...
We won't specify the details from Isabelle/HOL in this section but rather refer the reader to \textit{Concrete Semantics with Isabelle/HOL}\cite{nipkow} which shows an introduction to Isabelle/HOL and theorem proving.

On the other hand, Isabelle/HOL has also code generation facilities\cite{isabelle-codegen} that allow us to generate executable SML code from the HOL specification of our semantics.
This generated code will later on enable us to execute the semantics we define.
Also Isabelle/HOL allows for Isabelle/ML code to be embedded in the theories\cite{isabelle-implementation}, this facilitates the final translation to C process.
