...
Para modelar correctamente la semántica de las expresiones en Chloe, es necesario contar con esta distinción entre expresiones del lado izquierdo y expresioness del lado derecho.
En las siguientes secciones al referirse a expresiones del lado derecho se utilizará simplemente el nombre expresiones y se utilizará expresion LHS (por sus siglas en ingles \textit{left-hand-side}) en lugar de expresiones del lado izquierdo al referirse a dichas expresiones.
\begin{comment}
...
Tiene ``dereferencing'', ``referencing'' y acceso a arreglos, estos son, en C, los operadores \verb|*|, \verb|&| y \verb|[]|, respectivamente.
Finalmente, como expresiones LHS se tienen ``dereferencing'' y accesso a arreglos.
\begin{comment}
FIXME mosca! esto está mal traducido!
\end{comment}
...
Una opción para poder modelar una función de asignación de memoria que presente este tipo de comportamiento, es decidir ``non-deterministically'' que esta función puede retornar \verb|null|.
...
We will go to an erroneous state if our semantics encounters behavior that is undefined by the C standard\cite{c99}, an example of undefined behavior is integer overflow.
